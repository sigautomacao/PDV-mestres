...
$payments = array();
while ($payment = mysqli_fetch_assoc($sqlPayments)) {//looop para tratamento dos dados dos itens
	$paymentPD = array( //padrão de dados
		'id_pay' => floatval($payment['id_pay']),
		'descricao' => $payment['forma'],
		'valor' => floatval($payment['valor'])
	);

	array_push($payments, $paymentPD);
}


$vendaPD['desc_itens'] = $vdesc_itens; 
// $vendaPD['vdesc'] += $vdesc_itens;
$vendaPD['total'] -= $vdesc_itens; 

//linka os itens dentro da venda
$retorno = array(
	'venda' => $vendaPD,
	'itens' => $itens,
	'payments' => $payments
